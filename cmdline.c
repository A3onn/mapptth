...
/**
 * @brief add a node at the head of the queue 
 */
static void add_node(struct generic_queue** queue) {
    struct generic_queue* new_node = (struct generic_queue*) malloc(sizeof(struct generic_queue));
    new_node->next = *queue;
    *queue = new_node;
...
int check_multiple_option_occurrences(const char* prog_name, unsigned int option_given, unsigned int min, unsigned int max, const char* option_desc) {
...
int cmdline_parser_required2(struct gengetopt_args_info* args_info, const char* prog_name, const char* additional_error) {
    int error_occurred = 0;
 ...
 */
static int update_arg(void* field, char** orig_field,
...
/**
 * @brief store information about a multiple option in a temporary queue
 * @param queue where to (temporarily) store multiple options
 */
static int update_multiple_arg_temp(struct generic_queue** queue,
    ...
    while(1) {
        add_node(queue);
        if(update_arg((void*) &((*queue)->arg), &((*queue)->orig), 0,
...
/**
 * @brief free the passed queue (including possible string argument)
 */
static void free_queue(struct generic_queue* queue, short string_arg) {
    if(queue) {
        struct generic_queue* tmp;
        while(queue) {
            tmp = queue;
            if(string_arg && queue->arg.string_arg)
                free(queue->arg.string_arg);
            if(queue->orig)
                free(queue->orig);
            queue = queue->next;
            free(tmp);
...
/**
 * @brief updates a multiple option starting from the passed queue
 */
static void update_multiple_arg(void* field, char*** orig_field,
    unsigned int field_given, unsigned int prev_given, union generic_value* default_value,
    cmdline_parser_arg_type arg_type,
    struct generic_queue* queue) {
    int i;
    struct generic_queue* tmp;
        ...
        *orig_field = (char**) realloc(*orig_field, (field_given + prev_given) * sizeof(char*));
        ...
        case ARG_INT:
            *((int**) field) = (int*) realloc(*((int**) field), (field_given + prev_given) * sizeof(int));
            break;
        case ARG_LONG:
            *((long**) field) = (long*) realloc(*((long**) field), (field_given + prev_given) * sizeof(long));
            break;
        case ARG_STRING:
            *((char***) field) = (char**) realloc(*((char***) field), (field_given + prev_given) * sizeof(char*));
            break;
        default:
            break;
        };
        ...
        for(i = (prev_given - 1); i >= 0; --i) {
            ...
            case ARG_INT:
                ...
                break;
            case ARG_LONG:
                (*((long**) field))[i + field_given] = tmp->arg.long_arg;
                break;
            case ARG_STRING:
            ...
            (*orig_field)[i + field_given] = queue->orig;
            queue = queue->next;
            free(tmp);
        }
...
int cmdline_parser_internal(
    ...
    struct generic_queue* allowed_domains_queue = NULL;
    struct generic_queue* disallowed_paths_queue = NULL;
    struct generic_queue* allowed_extensions_queue = NULL;
    int error_occurred = 0;
    ...
    while(1) {
        ...
        case 'a': /* Allow the crawler to go to URLs found on other domains..  */
            ...
            if(update_multiple_arg_temp(&allowed_domains_queue,
        ...
        case 'd': /* Disallow the crawler to go to these directories..  */
            ...
            if(update_multiple_arg_temp(&disallowed_paths_queue,
        ...
        case 'x': /* The crawler will only fetch documents with these extensions, but if no extension is found in an URL, this filter won't apply. Extensions have to start with a '.' (dot)..  */
            ...
            if(update_multiple_arg_temp(&allowed_extensions_queue,
    ...
    update_multiple_arg((void*) &(args_info->allowed_domains_arg),
        ...
        ARG_STRING, allowed_domains_queue);
    update_multiple_arg((void*) &(args_info->disallowed_paths_arg),
        ...
        ARG_STRING, disallowed_paths_queue);
    update_multiple_arg((void*) &(args_info->allowed_extensions_arg),
        ...
        ARG_STRING, allowed_extensions_queue);
...
failure:
    free_queue(allowed_domains_queue, 1);
    free_queue(disallowed_paths_queue, 1);
    free_queue(allowed_extensions_queue, 1);
